...
function _updateTopbarForPdf(isPdf){
  ...
  // V2_70: 「PDFを開いている間はコマンド名を『画面検索』→『ページ検索』に変えてほしい」
  // との要望対応。ヘッダーの検索ボタン(#searchMenuBtn)のラベル・titleを、PDF表示中は
  // 「ページ検索」に、それ以外(DXF/Excel)では従来通り「画面検索」に切り替える
  var searchBtn238=document.getElementById('searchMenuBtn');
  if(searchBtn238){
    searchBtn238.title=isPdf?'ページ検索':'画面検索';
    var lbl238=searchBtn238.querySelector('.tb-tool-label');
    // V2_71: 「ページ検索」表記は1段目「ページ」・2段目「検索」で改行してほしいとの
    // 要望のため、<br>で明示的に2段に分ける（「画面検索」は従来どおり自動折返しのまま）
    if(lbl238) lbl238.innerHTML=isPdf?'ページ<br>検索':'画面検索';
  }
